...
- 给第三章到第五章生成图表计划，提前列出应生成的 Visio 图和第五章程序截图占位。
...
- 第5章需要真实程序截图；如果我没有提供截图，请在图表登记中保留 needs_user_screenshot 占位，不要伪造截图。
...
第4章生成 Visio 功能架构图、总体架构图、技术/部署结构图、总 E-R 图、单实体 E-R 图、关键模块流程图、数据库三线表。
第5章保留真实程序截图位置；没有截图时用 needs_user_screenshot 占位，不要生成假截图。
...
- 第4章规划功能架构图、总体架构图、技术/部署结构图、总 ER 图、单实体 ER 图、关键模块流程图。
- 第5章规划真实程序截图；如果我没有提供截图，登记 needs_user_screenshot 占位。
- 所有结构图都要保留 Visio .vsdx 源文件。
```

第五章截图不能伪造。如果项目不能运行、没有账号、缺少服务或用户还没提供截图，应该在 `figure-registry.yaml` 里登记待补截图，而不是生成看起来像系统界面的假图。
...
第四步：确认后按章节生成正文。
第五步：生成第3章用例图、业务流程图、需求结构图，以及第4章功能架构图、总体架构图、技术/部署结构图、ER 图、关键模块流程图、数据库三线表。
第六步：登记第5章真实程序截图或 needs_user_screenshot 占位。
第七步：导出 Word 草稿，并检查格式、证据和图表编号。
```
...
3. 第4章有架构图、功能图、ER 图、关键模块流程图、数据库表
4. 第5章有测试用例表和真实截图占位
5. 所有缺证据的地方标出来
...
```text
使用 thesis-standardizer，基于这个真实项目帮我生成本科毕业论文。先读代码和模板，再生成图表计划，不要编造证据；第3章生成用例图、业务流程图和需求结构图，第4章生成总体架构图、功能架构图、总 ER 图、单实体 ER 图、关键模块流程图和数据库三线表，第5章保留真实程序截图或 needs_user_screenshot 占位，最后给我 Word 草稿和缺证据清单。
```
