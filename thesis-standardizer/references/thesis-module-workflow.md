...
6. Delivery checks
   ...
   - For final `.docx`, use template compare/repair scripts only when a final document and template are available.
...
- database design three-line tables via `docx-production-rules.md`
...
## Standalone AIGC Module

Use only on request. Run `run_aigc_repair_loop.py`, revise high-risk paragraphs first, and preserve all facts and references.
