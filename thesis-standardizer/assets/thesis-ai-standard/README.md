...
10. 使用 `drawio/` 下的 `.drawio` 模板重画结构性图；需要 Visio 可编辑 E-R 图、用例图、流程图或功能架构图源时，先用对应 `layout_*_diagram.py` 自动布局，再用 `generate_visio_*_diagram.ps1` 生成 `.vsdx` 和 `.png`。
...
- 专属图形：复制 `drawio/` 模板后按论文对象改节点，或用 Visio 脚本生成 `.vsdx` 可编辑图源；总 E-R 图应限量展示核心实体、代表属性和主关系，用例图应按角色拆分并保留系统边界或参与者关系，流程图应使用开始/结束、处理、判断和回路箭头表达业务逻辑，功能架构图应使用系统根节点、前后台模块和角色/模块功能叶子的树状结构。
