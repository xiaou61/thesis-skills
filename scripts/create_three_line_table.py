...
Usage:
    ...
    python scripts/create_three_line_table.py unpacked/ "1-1" "符号说明" \\
        --headers "符号,说明" \\
        --rows '[["S","状态空间"],["A","动作空间"]]' \\
        --anchor "以下是符号说明"

    # Print XML snippet only (no file modification):
    python scripts/create_three_line_table.py --caption "表 1-1 示例" \\
        --headers "列1,列2,列3" \\
        --rows '[["a","b","c"],["d","e","f"]]'

    # Custom column widths (DXA, must sum to 9070):
    python scripts/create_three_line_table.py unpacked/ "2-1" "参数说明" \\
        --headers "参数,取值,说明" \\
        --rows '[["lr","0.001","学习率"]]' \\
